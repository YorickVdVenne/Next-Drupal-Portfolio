...
export default function Experience(props: ExperienceProps): JSX.Element {
  const [activeListItem, setActiveListItem] = useState(
    props.experienceData.companies[0].name
  );
  const activeListItemIndex = useMemo(() => {
    return props.experienceData.companies
      .map((company) => company.name)
      .indexOf(activeListItem);
  }, [activeListItem, props.experienceData.companies]);

  return (
    <Section maxWidth={700}>
      <NumberedHeading id={props.experienceData.bookmark} number={2}>
        {props.experienceData.title}
      </NumberedHeading>
      <div className={gridStyles.grid}>
        <div className={styles.tabListWrapper}>
          <TabList
            items={props.experienceData.companies.map(
        ...
        <div className={styles.tabPannelWrapper}>
          <TabPanel
            data={props.experienceData.jobs}
