            ...
            <tr>
              <td className={styles.techData} colSpan={2}>
                {project.technologies.map((tech: Technologies, key: number) => (
                  <span className={styles.techTag} key={key}>
                    {tech.name}
                  </span>
                ))}
              </td>
        ...
        <div className={styles.description}>
          ...
          <div className={styles.externalLinks}>
            ...
            {hasValue(project.githubLink) && (
              <Button as="link" href={project.githubLink} icon="github">
                {t("button.viewCode")}
              </Button>
            )}
          </div>
          ...
          <div className={styles.screenshotsContainer}>
            ...
            <Slider activeItemIndex={setActiveImageIndex}>
              {project.screenshots.map(
                (screenshot: MediaImage, key: number) => (
            ...
            <div className={styles.screenshotDetails}>
              {project.screenshots.map(
                (screenshot: MediaImage, key: number) => (
                  <div
                    key={key}
                    className={clsx(styles.screenshotDetail, {
                      [styles.active]: key === activeImageIndex,
                    })}
                  >
