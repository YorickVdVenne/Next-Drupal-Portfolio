...
export const InternalOrExternalLink = ({
  ...
  if (
    ...
    return <a href={href} target={target ?? '_self'} {...props}>{children}</a>
  }

  return (href !== '' ? <Link href={href} target={target ?? '_self'} rel='noopener' {...props}><>{children}</></Link> : <p className={props.className}>{children}</p>)
}
